...
import { MOONBASE_GREETER_ADDRESS } from '../constants/index.js';
import callPermitABI from '../constants/callPermitABI.js';
import GreeterContractABI from "../artifacts/contracts/Greeter.sol/Greeter.json";
import { networks } from '../hardhat.config.js';


const provider = new ethers.JsonRpcProvider(networks.moonbase.url, {
...
const GreeterInterface = new ethers.Interface(GreeterContractABI);
const data = GreeterInterface.encodeFunctionData('setGreeting', [
  "Meow" // Voyage V2 contract
]);

const gasEstimate = await provider.estimateGas({
  from: userSigner.address,
  to: MOONBASE_GREETER_ADDRESS, // Cartographer V1 contract
  data,
});
